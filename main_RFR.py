...
def train_and_predict(code, today_data):
    ...
    data.drop(['日期', '股票代码', '名称', '前收盘',
               '涨跌额', '涨跌幅', '换手率', '成交量', '成交金额', '流通市值', '总市值'], axis=1, inplace=True)
    ...
    # 预测明天的情况
    # 明天的涨幅
    score_result = get_score_result(x, data.iloc[:, len(data.columns) - 2:len(data.columns)], today_data)
    score = score_result[0]
    # t_predict_zhangfu = score_result[1][0][0]
    t_predict_high = score_result[1][0][0]
    t_predict_low = score_result[1][0][1]
    ...
    if (zhangfu > 3) or (is_my_code):
        with open(os.path.join(end_date + "\\predict_linear", gupiao_name + '.csv'.format(code=code)), 'w',
                  encoding='utf-8') as f:
            f.write(result)

        all_result = gupiao_name + "[" + str(t_predict_low) + "-" + str(t_predict_high) + "],精确率：" + str(
            score) + "能赚" + str(zhangfu) + "\n"
...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(max_workers=worker_num) as tpe:
        tpe.map(download_code_data,
                [(code, start_date, end_date, data_path, predict_path) for code in all_code])
