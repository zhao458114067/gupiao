    ...
    for url_template in code_download_path_urls:
        ...
        if len(text) >= code_data_length_limit:
            for i in range(len(text) - 2, predit_days_number, -1):
                ...
                nextShou = text[i - predit_days_number].split(",")[4]
...
def get_score_result(x, y, today_data, type):
    ...
    # if type == 'shou':
    grid_search = RandomForestRegressor(n_estimators=450, max_features=1, random_state=42, n_jobs=-1)
    # if type == 'di':
...
def train_and_predict(code, today_data):
    ...
    data.drop(['日期', '股票代码', '名称'], axis=1, inplace=True)
    ...
    if (t_predict_zhangfu > 1) or (is_my_code):
