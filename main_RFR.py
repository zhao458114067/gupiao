...
def download_code_data(*args):
    ...
    for url_template in code_download_path_urls:
        ...
        if len(text) >= code_data_length_limit:
            ...
            if float(price) > 60 or float(today_huan) > 12 or float(today_huan) < 1.5:
                continue
...
def get_score_result(x, y, today_data, type):
...
def train_and_predict(code, today_data):
    ...
    data.drop(['日期', '股票代码', '名称',
               '涨跌额', '涨跌幅', '成交金额', '总市值'], axis=1, inplace=True)
    ...
    # 预测明天的情况
    # 明天的涨幅
    score_result = get_score_result(x, data.iloc[:, len(data.columns) - 3:len(data.columns) - 2], today_data, "zhangfu")
    score_zhangfu = score_result[0]
    t_predict_zhangfu = score_result[1][0][0]
    score_result = get_score_result(x, data.iloc[:, len(data.columns) - 2:len(data.columns) - 1], today_data, "shou")
    score_high = score_result[0]
    t_predict_high = score_result[1][0][0]
    score_result = get_score_result(x, data.iloc[:, len(data.columns) - 1:len(data.columns)], today_data, "di")
    score_low = score_result[0]
    t_predict_low = score_result[1][0][0]
    ...
    result = "股票编号：" + code + ",下个交易日会" + result_type + str(t_predict_zhangfu) + "\n股票名称：" + gupiao_name + "\n收盘：" + \
             str(t_predict_high) + ",精确率：" + str(score_high) + "\n" + "低点：" + str(t_predict_low) + ",精确率：" + str(
        score_low) + "\n浮动：" + str(fudong) + "\n跌幅：" + str(diefu) + "\n涨幅：" + str(zhangfu)

    if (t_predict_high > today_price and t_predict_zhangfu > 3 and diefu < 3) or (
            is_my_code):
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        print(all_code)
    start_date = (date.today() - timedelta(days=code_data_length_limit * 1.6)).strftime("%Y%m%d")
