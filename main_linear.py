...
def train_and_predict(code, today_data):
    ...
    # 取特征值、目标值
    x = data.iloc[:, [0, 3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14]]
    y = data["涨跌幅"]
    # 划分训练集
    ...
    x_today = x_today.iloc[:, [0, 3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14]]
    x_today = transfer.transform(x_today)
    t_predict = esetimator.predict(x_today)
    if t_predict[0] > 0:
        result = "股票编号：" + code + "\r\n股票名称：" + today_data.split(",")[2] + "\r\n下个交易日会涨" + str(t_predict) + "，精确率：" + str(
            score)
    else:
        result = "股票编号：" + code + "\r\n股票名称：" + today_data.split(",")[2] + "\r\n下个交易日会跌" + str(t_predict) + "，精确率：" + str(
            score)
    print(result)
    if score > 0.7:
...
if __name__ == '__main__':
    ...
    download_code_data("688777", start_date, end_date, data_path, predict_path)
